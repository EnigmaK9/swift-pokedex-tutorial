...
\midrule
\texttt{App/} & \texttt{PokedexTutorialApp.swift} — @main entry point \\
\texttt{Models/} & \texttt{Pokemon.swift} — core entity, custom JSON decoder \\
                  & \texttt{PokemonType.swift} — 18-type enum with colors \\
                  & \texttt{PokemonDetail.swift} — stats, abilities, sprites \\
                  & \texttt{LoadingState.swift} — generic async state enum \\
\texttt{Services/} & \texttt{PokemonServiceProtocol.swift} — API contract \\
                  & \texttt{PokemonAPIService.swift} — real Pok\'{e}API client \\
                  & \texttt{MockPokemonService.swift} — local fixture data \\
\texttt{ViewModels/} & \texttt{PokemonListViewModel.swift} — search, filter, pagination \\
                  & \texttt{PokemonDetailViewModel.swift} — detail \& type relations \\
\texttt{Views/} & \texttt{PokemonListView.swift} — main screen (List/Grid) \\
                  & \texttt{PokemonDetailView.swift} — detail with FlowLayout \\
                  & \texttt{PokemonRowView.swift} — reusable row component \\
                  & \texttt{TypeBadgeView.swift} — colored type capsule \\
                  & \texttt{StatBarView.swift} — color-coded stat bar \\
                  & \texttt{FilterChipView.swift} — selectable type chip \\
\texttt{Stores/} & \texttt{FavoritesStore.swift} — UserDefaults persistence \\
\texttt{Extensions/} & \texttt{Color+PokemonType.swift}, \texttt{View+Extensions.swift} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{minipage}{0.85\textwidth}
\small
\textbf{Tests:} \texttt{PokedexTutorialTests/} (24 tests: model, ViewModel, API)\\
\textbf{Resources:} \texttt{Assets.xcassets/} (app icon), \texttt{mock\_pokemon\_list.json} (sample data)
\end{minipage}
}
\end{center}
